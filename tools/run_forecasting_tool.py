...
class RunForecastStepFunction:
	...
	def __init__(self, **kwargs):
		if not self.config_file_name.endswith(".json"):
			raise ValueError("Invalid config file format. Must be a JSON file.")
		if not self.data_file_name.endswith(".csv"):
	...
	def stage_input_files(self) -> RunForecastStepFunction:
		...
		RunForecastStepFunction.stage_data_to_s3(
			body = csv_buffer.getvalue(),
			key = f"s3://{bucket}/{self.process_id}/input/{self.data_file_name}"
		)
		RunForecastStepFunction.stage_data_to_s3(
			body = config,
			key = f"s3://{bucket}/{self.process_id}/input/{self.config_file_name}"
		)
